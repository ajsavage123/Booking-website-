...
export default function BookPage() {
  ...
  // Fetch booked slots when date/service changes
  useEffect(() => {
  ...
  async function fetchBookedSlots(date: string) {
    ...
    try {
      const res = await fetch(
        `/api/slots?date=${date}&service=${form.service}`
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50/50 via-white to-teal-50/30">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          {STEP_LABELS.map(({ n, label, icon: Icon }, i) => (
            <div key={n} className="flex items-center">
              <div className="flex flex-col items-center">
                ...
                <span
                  className={`text-xs font-medium mt-1.5 hidden sm:block ${
                    step === n ? "text-teal-600" : "text-slate-400"
                  }`}
                >
                  {label}
                </span>
              ...
              {i < STEP_LABELS.length - 1 && (
                <div
                  className={`h-0.5 w-12 sm:w-16 mx-1 sm:mx-2 rounded transition-colors ${
                    step > n ? "bg-teal-400" : "bg-slate-200"
                  }`}
                />
              )}
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-teal-100/60 overflow-hidden animate-scale-in">
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {SERVICES.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setForm((f) => ({ ...f, service: s.id, time: "" }))}
              ...
              <div className="flex justify-center">
                <DayPicker
                  ...
                  classNames={{
                    ...
                    caption: "font-display text-lg font-semibold text-slate-800",
                  }}
                />
              </div>
          ...
          {step === 3 && (
            <div className="p-6 md:p-8">
              ...
              <p className="text-slate-500 text-sm mb-6">
                Available slots for{" "}
                {form.date && format(new Date(form.date + "T00:00:00"), "EEEE, MMM d")}
              </p>
                    ...
                    {ALL_TIME_SLOTS.map((slot) => {
                      ...
                      return (
                        <button
                          key={slot}
                          onClick={() =>
                            !isBooked && setForm((f) => ({ ...f, time: slot }))
                          }
                          disabled={isBooked}
                          className={`py-3 px-2 rounded-xl text-sm font-medium transition-all ${
                            isSelected
                              ...
                              : "bg-slate-50 text-slate-700 border border-slate-200 hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700"
                          }`}
                        >
                          {formatTime(slot)}
                        </button>
          ...
          {step === 4 && (
            <div className="p-6 md:p-8">
              ...
              <p className="text-slate-500 text-sm mb-6">
                Almost done! Please confirm your information.
              </p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`input-ring w-full px-4 py-3 rounded-xl border text-sm bg-slate-50 focus:bg-white transition-colors ${
        ...
        <div className="mt-6 flex items-center justify-center gap-6 text-xs text-slate-400">
          {["Secure booking", "Free cancellation", "No credit card needed"].map(
            (t) => (
